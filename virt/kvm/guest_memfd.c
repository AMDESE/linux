				       ...
				       loff_t size, u64 flags)
{
	...
	m = flags & GUEST_MEMFD_FLAG_INIT_SHARED ? SHAREABILITY_ALL :
						   SHAREABILITY_GUEST;
					      ...
					      struct conversion_work *work)
{
	...
	list_for_each_entry(gmem, gmem_list, entry)
		__kvm_gmem_invalidate_begin(gmem, invalidate_start, invalidate_end);
}
					    ...
					    struct conversion_work *work)
{
	...
	gmem_list = &inode->i_mapping->i_private_list;
	list_for_each_entry(gmem, gmem_list, entry)
		__kvm_gmem_invalidate_end(gmem, invalidate_start, invalidate_end);
}
						      ...
						      struct folio *first_folio)
{
	...
	if (kvm_gmem_allocator_ops(inode)->skip_merge_folio &&
	    kvm_gmem_allocator_ops(inode)->skip_merge_folio(first_folio))
		return 0;
...
int kvm_gmem_create(struct kvm *kvm, struct kvm_create_guest_memfd *args)
{
	...
	u64 valid_flags = kvm_gmem_get_supported_flags(kvm);
